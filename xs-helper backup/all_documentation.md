...
# ArrayLinearRegSlope -  (系統函數)（Array函數）
...
# GetBarOffsetForYears -  (系統函數)（邏輯判斷）
...
```xs
Value1 = GetField("停止轉換結束日");
Value1 = GetField("StopConvertingED");
```
...
欄位格式為西元年月日，例如20250910

請注意：這個日期可能是「未來」的日期（例如已經進入停止轉換期間，可是尚未結束）也可能是「過去」的日期（例如已經結束停止轉換）

可以檢查GetFieldDate("停止轉換結束日")是否等於0來判斷是否有資料。
...
```xs
Value1 = GetField("漲停委買數量",param:=整體市場);
Value1 = GetField("UpLimitBidUnits",param:=整體市場);
```
...
可以用 GetField("漲停委買數量", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("漲停委買筆數",param:=整體市場);
Value1 = GetField("UpLimitBidTx",param:=整體市場);
```
...
可以用 GetField("漲停委買筆數", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("漲停委賣數量",param:=整體市場);
Value1 = GetField("UpLimitAskUnits",param:=整體市場);
```
...
可以用 GetField("漲停委賣數量", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("漲停委賣筆數",param:=整體市場);
Value1 = GetField("UpLimitAskTx",param:=整體市場);
```
...
可以用 GetField("漲停委賣筆數", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("跌停委買筆數",param:=整體市場);
Value1 = GetField("DownLimitBidTx",param:=整體市場);
```
...
可以用 GetField("跌停委買筆數", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("跌停委賣數量",param:=整體市場);
Value1 = GetField("DownLimitAskUnits",param:=整體市場);
```
...
可以用 GetField("跌停委賣數量", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("跌停委賣筆數",param:=整體市場);
Value1 = GetField("DownLimitAskTx",param:=整體市場);
```
...
可以用 GetField("跌停委賣筆數", "D", param:= "股票") 的方式來取得個別資訊。
...
```xs
Value1 = GetField("合約負債");
Value1 = GetField("ContractLiabilities");
